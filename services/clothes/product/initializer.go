...
func Intialize(curDb *db.Database, cfg *config.Config, validator *validation.Validator, api router.Router) {
	...
	productTemplateRepo := NewProductTemplateRepo(curDb, productConfig.DBCollection.ProductTemplate)
	...
	productService := NewService(productListRepo, productDetailRepo, productBatchRepo, productTemplateRepo, txn)
